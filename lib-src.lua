...
library.CreateLoader("akiri", Vector2.new(300, 250));
local window = library.Window("", Vector2.new(524, 550));

local playersTab = window:Tab("Players");
local updateUI = playersTab:AddESPPreview()
local enemySection = playersTab:Section("Enemies", "Left");
local testSection = playersTab:Section("Document Test", "Right");
...
testSection:Slider({
    ...
    Min = 0,
    Symbol = "",
    Callback = function(retard)
   print(retard .. "theres that many retard splittas :cute:")
    end
});
local toggle_test = testSection:Toggle({Title = "Toggle Testing", Flag = "Test", Callback = function(a)
    print(a)
end});
    toggle_test:Keybind({
...
testSection:Dropdown({
    Title = "Theme",
    List = {"Default", "frame", "splitta", "din", "vault", "lucii", "lxyo", "finobe", "argy"},
    Default = "Default",
    Callback = function(v)
if v  ==  "frame" then 
print("frame is my lord uwu ")
    else 
        print(v.. " is retarded :angry:")
        end
    end
});
enemySection:Toggle({Title = "Enabled", Flag = "EnabledESP"})
enemySection:Toggle({Title = "Name", Flag = "ESPName"}):Colorpicker({Title = "Name", Color = Color3.fromRGB(255, 255, 255), Flag = "ESPNameColor"})
enemySection:Toggle({Title = "Bounding Box", Flag = "ESPBoundingBox"}):Colorpicker({Title = "Bounding Box", Color = Library.Theme.Accent[1], Flag = "ESPBoundingBoxColor"})
enemySection:Toggle({Title = "Health Bar", Flag = "ESPHealthBar"}):Colorpicker({Title = "Health Bar", Color = Color3.fromRGB(0, 255, 0), Flag = "ESPHealthBarColor"})
enemySection:Toggle({Title = "Distance", Flag = "ESPDistance"}):Colorpicker({Title = "Distance", Color = Color3.fromRGB(255, 255, 255), Flag = "ESPDistanceColor"})
enemySection:Toggle({Title = "Weapon", Flag = "ESPWeapon"}):Colorpicker({Title = "Distance", Color = Color3.fromRGB(255, 255, 255), Flag = "ESPWeaponColor"})

local dummyHealth, dummyMaxHealth, dummyPlus = 100, 100, false

Utility.AddConnection(RunService.RenderStepped, function()
    do
        if (dummyHealth == 1) then
            dummyPlus = "Up"
        elseif (dummyHealth == 100) then
            dummyPlus = "Down"
        end
        if (dummyPlus == "Up") then
            dummyHealth = dummyHealth + 1
        else
            dummyHealth = dummyHealth - 1
        end
    end
    updateUI:UpdateName(library.Flags.EnabledESP and library.Flags.ESPName, library.Flags.ESPNameColor)
    updateUI:UpdateDistance(library.Flags.EnabledESP and library.Flags.ESPDistance, library.Flags.ESPDistanceColor)
    updateUI:UpdateWeapon(library.Flags.EnabledESP and library.Flags.ESPWeapon, library.Flags.ESPWeaponColor)
    updateUI:UpdateHealthBar(library.Flags.EnabledESP and library.Flags.ESPHealthBar, library.Flags.ESPHealthBarColor, {
        Health = dummyHealth,
        MaxHealth = dummyMaxHealth
    })
    updateUI:UpdateBoundingBox(library.Flags.EnabledESP and library.Flags.ESPBoundingBox, library.Flags.ESPBoundingBoxColor)
end)
